...
# =================================================================
#               MODELO REVERSÍVEL COM ÁGUA 
# =================================================================
def modelo_reversivel(t, k_f, K_A, K_B, k_r, K_D):
    ...
    def dXdt(t, X):
        ...
        numerador = k_f * K_A * K_B * C_A * C_B - k_r * K_D * C_D
...
# =================================================================
params_iniciais = [0.1, 0.1, 0.1, 0.01, 0.1]  # k_f, K_A, K_B, k_r, K_D
bounds = ([1e-5, 1e-3, 1e-3, 1e-5, 1e-3], [1e3, 1e3, 1e3, 1e3, 1e3])  # Limites
...
try:
    popt, pcov = curve_fit(
        ...
        p0=params_iniciais,
        bounds=bounds,
        maxfev=100000  # Aumento de iterações
    )
    k_f, K_A, K_B, k_r, K_D = popt
    print("Ajuste bem-sucedido!")
    print(f"k_f = {k_f:.3e}, K_A = {K_A:.3f}, K_B = {K_B:.3f}")
    print(f"k_r = {k_r:.3e}, K_D = {K_D:.3f}")
